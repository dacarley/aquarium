...
type PGA = 6144 | 4096 | 2048 | 1024 | 512 | 256;
type SPS = 8 | 16 | 32 | 64 | 128 | 250 | 475 | 860;

export default class ADS1x15 {
    private static address = 0x48;
    private static i2c = i2cBus.openSync(1);
    ...
    public static async readSingle(channel: number, pga: PGA = 6144, sps: SPS = 250): Promise<number> {
        ...
        this.i2c.writeWordSync(this.address, ADS1015_REG_POINTER_CONFIG, this._encodeWord(config));
    ...
    private static _getConfig(channel: number, pga: PGA, sps: SPS): number {
    ...
    private static _encodeWord(value: number): number {
    ...
    private static _decodeWord(word: number): number {
        const msb = word & 0xFF;
